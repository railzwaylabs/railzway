...
func NewUsageIngestLimiter(cfg config.Config, client *redis.Client) (*UsageIngestLimiter, error) {
	...
	if client == nil {
		// Should not happen if FX provides it, but good safety check if module loaded without redis
