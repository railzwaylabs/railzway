...
type Repository interface {
	...
	ListOverdueInvoices(ctx context.Context, orgID snowflake.ID, currency string, now time.Time, pageToken string, pageSize int) ([]OverdueInvoiceRow, error)
	ListOutstandingCustomers(ctx context.Context, orgID snowflake.ID, currency string, now time.Time, pageToken string, pageSize int) ([]OutstandingCustomerRow, error)
	ListPaymentIssues(ctx context.Context, orgID snowflake.ID, now time.Time, pageToken string, pageSize int) ([]PaymentIssueRow, error)
	LoadActionSummary(ctx context.Context, orgID snowflake.ID, currency string, now time.Time) (ActionSummaryRow, error)
	ListCollectionQueue(ctx context.Context, orgID snowflake.ID, currency string, now time.Time, pageToken string, pageSize int) ([]CollectionQueueRow, error)
	ListFailedPaymentActions(ctx context.Context, orgID snowflake.ID, currency string, now time.Time, pageToken string, pageSize int) ([]FailedPaymentActionRow, error)
	...
	// IA Methods
	ListInboxItems(ctx context.Context, orgID snowflake.ID, pageToken string, pageSize int, now time.Time) ([]InboxRow, error)
	ListMyWorkItems(ctx context.Context, orgID snowflake.ID, userID string, pageToken string, pageSize int, now time.Time) ([]MyWorkRow, error)
	ListRecentlyResolvedItems(ctx context.Context, orgID snowflake.ID, userID string, pageToken string, pageSize int, since time.Time) ([]ResolvedRow, error)
