		...
		OnStart: func(ctx context.Context) error {
			...
			go func() {
				...
				for {
					...
					periods, err := repo.ListOpenSubscriptionPeriods(context.Background(), baseNow, batchSize)
					if err != nil {
						logger.Warn("subscription close period failed", zap.Error(err))
					} else {
						result := ClosePeriodResult{}
						for _, period := range periods {
							generated, err := job.closeOne(context.Background(), period, baseNow)
							if err != nil {
								logger.Warn("subscription close period failed", zap.Error(err))
								break
							}
					...
					if gracePeriod > 0 {
						opened, err := openDueDraftInvoices(context.Background(), db, invoiceSvc, baseNow, gracePeriod, batchSize)
						if err != nil {
...
func openDueDraftInvoices(ctx context.Context, db *gorm.DB, invoiceSvc invoicedomain.Service, baseNow time.Time, grace time.Duration, limit int) (int, error) {
	if db == nil || invoiceSvc == nil {
		return 0, nil
	}
	cutoff := baseNow.UTC().Add(-grace)
	if limit <= 0 {
		...
		`SELECT id, org_id, period_end
		 ...
		 WHERE status = ?
		   ...
		   AND NOT EXISTS (
		       SELECT 1
		       FROM customers
		       WHERE customers.org_id = invoices.org_id
		         AND customers.id = invoices.customer_id
		         AND customers.test_clock_id IS NOT NULL
		   )
	...
	for _, row := range rows {
		if baseNow.UTC().Before(row.PeriodEnd.Add(grace)) {
			continue
		}
