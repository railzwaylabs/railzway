...
func (r *repo) Count(ctx context.Context, db *gorm.DB, orgID snowflake.ID) (int64, error) {
	var count int64
	// Count active subscriptions (not canceled)
	err := db.WithContext(ctx).
		Model(&subscriptiondomain.Subscription{}).
		Where("org_id = ? AND status != ?", orgID, subscriptiondomain.SubscriptionStatusCanceled).
		Count(&count).Error
	if err != nil {
		return 0, err
	}
