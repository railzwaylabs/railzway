...
func (r *repository) Upsert(ctx context.Context, clock domain.TestClock) error {
	return r.db.WithContext(ctx).Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"status", "clock_time", "updated_at"}),
	}).Create(&clock).Error
}

func (r *repository) Update(ctx context.Context, orgID uuid.UUID, updates map[string]interface{}) error {
	if len(updates) == 0 {
		return nil
	}
