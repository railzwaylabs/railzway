...
export default function OrganizationSessions() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      setLoading(true)
      const resp = await api.organizations.listSessions(orgId)
      setSessions(resp.sessions || [])
    } catch (err) {
      toast.error("Failed to load organization sessions", err instanceof Error ? err.message : undefined)
    } finally {
      setLoading(false)
    }
  }, [orgId])
  ...
  const confirmRevoke = useCallback(async () => {
    if (!orgId || !selectedSession) return
    try {
      setBusyId(selectedSession.id)
      const resp = await api.organizations.revokeSession(orgId, selectedSession.id)
  ...
  }, [loadSessions, orgId, selectedSession])
    ...
    () => [
      {
        ...
        render: (session: AdminSession) => (
          <div>
            <div style={{ fontWeight: 600 }}>{session.displayName || session.email || session.userId}</div>
  ...
  return (
    <div className="page-content">
      <PageHeader
        ...
        description={`Review active admin sessions for this organization, including device and IP metadata. ${activeCount} session(s) currently active.`}
      />
      ...
      <DataTable
        ...
        data={sessions}
        loading={loading}
        emptyTitle="No organization sessions"
        emptyDesc="Sessions appear here when members access this organization in the admin UI."
      />
      ...
      >
        <DialogContent style={{ maxWidth: 480 }}>
          <DialogHeader>
            <DialogTitle>
              {selectedSession?.current ? "Log out current session" : "Revoke organization session"}
            </DialogTitle>
            <DialogDescription>
              {selectedSession
                ? `This will revoke the admin session for ${selectedSession.email || selectedSession.displayName || "this user"}${selectedSession.current ? " and sign out the current browser immediately." : "."}`
          ...
          <DialogFooter style={{ marginTop: 24 }}>
            ...
            <Button
              ...
              disabled={Boolean(selectedSession && busyId === selectedSession.id)}
            >
              {selectedSession?.current ? "Log out" : "Revoke Session"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
