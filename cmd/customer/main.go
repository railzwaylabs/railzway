...
func main() {
	app := fx.New(
		...
		entitlement.Module,
		usage.Module,
		fx.Provide(invoicerepo.NewRepository, invoiceservice.NewService),
		...
		invoiceviewerapi.Module, // unauthenticated invoice viewer (/public/*)
		publicapi.Module,        // API key-authenticated (/api/v1/*)
...
func newRouter(
	cfg *config.Config,
	customerHandler *customerportalhandler.Handler,
	publicHandler *publichandler.Handler,
	...
	publichandler.RegisterRoutes(router, publicHandler)
	apihandler.RegisterRoutes(router, apiHandler)

	distDir := filepath.Join("apps", "customer", "dist")
	if info, err := os.Stat(distDir); err == nil && info.IsDir() {
		...
		logger.Info("customer portal static serving enabled", zap.String("dir", distDir))
	} else {
		logger.Warn("customer portal dist not found, skipping static serve", zap.String("dir", distDir))
	}
		...
		OnStart: func(ctx context.Context) error {
			go func() {
				...
				if err != nil && err != http.ErrServerClosed {
					logger.Error("customer server stopped", zap.Error(err))
				}
			}()
			return nil
		},
		OnStop: func(ctx context.Context) error {
			logger.Info("customer server stopping", zap.String("addr", addr))
