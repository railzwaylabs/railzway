...
func registerTelemetryLifecycle(lc fx.Lifecycle, cfg *config.Config, logger *zap.Logger) {
	...
	if cfg != nil && strings.TrimSpace(cfg.App.Name) != "" {
		serviceName = strings.TrimSpace(cfg.App.Name) + "-api"
	}
...
func newRouter(cfg *config.Config, apiHandler *apihandler.Handler, logger *zap.Logger, dbConn *gorm.DB) *gin.Engine {
	...
	router.Use(httpmiddleware.SecurityHeadersWithCSP(cfg, cfg.App.CSPExtraDirectives, httpmiddleware.CSPProfilePublic))
...
func startHTTPServer(lc fx.Lifecycle, cfg *config.Config, router *gin.Engine, logger *zap.Logger) {
	...
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			go func() {
				logger.Info("api server started", zap.String("addr", addr), zap.String("tls_mode", string(cfg.App.TLSMode)))
				var err error
